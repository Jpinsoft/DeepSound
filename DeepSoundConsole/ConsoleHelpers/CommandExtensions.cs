...
namespace DeepSoundConsole.ConsoleHelpers
{
    public static class CommandExtensions
    {
        ...
        public static readonly Argument<string> ActionArgument = new Argument<string>("action") { Description = "Action to perform: 'analyze or a' (default), 'encode or e', 'decode or d'." };

        public static readonly Option<string> EncodeFormatOpt = new Option<string>("-f", "-F") { Description = "Encoder output audio format: Supported formats: wav (default), flac, ape." };
...
EXAMPLE (ANALYZE): DeepSoundConsole.exe ""D:\INPUT_FOLDER"" analyze -r
        ...
        public static void SetDeepSoundConsoleAction(this RootCommand rootCommand)
        {
            rootCommand.SetAction(parseResult =>
            {
                ...
                switch (action ?? "ANALYZE")
                {
                    ...
                    default: throw new ArgumentException($"Invalid action: {action}. Supported actions are 'analyze or a', 'encode or e', 'decode or d'.");
                }
            });
        }
    }
